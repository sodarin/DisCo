...
class Diffusion(nn.Module):
    def __init__(self, args, fp_encoder, num_timesteps=1000, n_class=10, fp_dim=512, device='cuda', beta_schedule='cosine',
        ...
        self.diffusion_encoder = GNN(args.dataset_num_features, args.hidden_dim, args.num_gc_layers, self.n_class, args, device).to(device)
    ...
    def forward_t(self, y_0_batch, data, t, fp_x, fq_x=None, domain=None):
        ...
        x_embed_batch, _, _ = self.diffusion_encoder(data.x, data.edge_index, data.batch, data.num_graphs, domain)
    ...
    def reverse_ddim(self, data, stochastic=True, fp_x=None, fq_x=None, domain=None):

        data = data.to(self.device)
        with torch.no_grad():

            if fp_x is None:
                fp_x, _, _ = self.fp_encoder(data.x, data.edge_index, data.batch, data.num_graphs, domain)
